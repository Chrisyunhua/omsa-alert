...
NONGLOBALHOTSPARE_HEALTHY_VALUES = {
    "Status": ["Ok"],
    "State": ["Online"],
}

GLOBALHOTSPARE_HEALTHY_VALUES = {
    "Status": ["Ok"],
    "State": ["Ready"],
}
    ...
    def check_for_problems(self, content):
        ...
        for disk in disks:
            if disk["Hot Spare"] == "Global":
                error_message = omsaalert.utility.check_expected_values(
                    disk, GLOBALHOTSPARE_HEALTHY_VALUES
                )
            else:
                error_message = omsaalert.utility.check_expected_values(
                    disk, NONGLOBALHOTSPARE_HEALTHY_VALUES
                )
